...
def match_template_in_folders(root_folder, template_folder_path, color, threshold=0.85):
    ...
    for filename in os.listdir(template_folder_path):
        if filename.endswith(('.jpg', '.jpeg', '.png')):  # Adjust file extensions as needed
            template_image_path = os.path.join(template_folder_path, filename)
            break
...
template_folder_path = r"E:\AI\Extra\gitCheck\main\destination_folder/"  # Replace with the actual path to the folder containing the template image
...
match_template_in_folders(root_folder_path, template_folder_path, color_to_search, matching_threshold)
